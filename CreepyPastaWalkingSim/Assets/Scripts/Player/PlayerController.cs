...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField] private float staminaChargeRatio = 1f;
    [SerializeField] private float staminaDepleteRatio = 1f;

    private bool staminaCharging = false;
    ...
    private GUIManager hud;

    private float SprintSpeed { get { return defaultSpeed * sprintMultiplier; } }
    /// <summary>
    /// return true if player is currently sprinting
    /// </summary>
    private bool Sprinting { get { return currentMovementSpeed == SprintSpeed; } }
    ...
    private void Awake()
    {
        ...
        hud = FindObjectOfType<GUIManager>();
    }
    ...
    private void StaminaTick()
    {
        if(staminaCharging == false)
        {
            ...
            if(Sprinting == true)
            {
                if(currentStamina > 0)
                {
                    currentStamina -= staminaDepleteRatio * Time.deltaTime;
                    if(currentStamina <= 0)
                    {
                        currentStamina = 0;
                        staminaCharging = true;
                    ...
                    hud.UpdateStaminaFill(currentStamina);
                }
            }
        }
        else
        {
            ...
            if(currentStamina < 1)
            {
                currentStamina += staminaChargeRatio * Time.deltaTime;
                if(currentStamina >= 1)
                {
                    currentStamina = 1;
